(() => {
  // 1. Site-specific selectors for article body container
  const domain = window.location.hostname.replace(/^www\./, '');
  const bodySelectors = {
    'nytimes.com':        '[data-testid="article-body"],.StoryBodyCompanionColumn,#article-body',
    'cn.nytimes.com':     '[class*="article-body"],[class*="article-content"],.article-body',
    'wsj.com':            '[data-type="paragraph"]',
    'cn.wsj.com':         '[data-type="paragraph"]',
    'theatlantic.com':    '[class*="article-body"],article section',
    'economist.com':      '[class*="article-body"],[class*="blog-post"]',
    'ft.com':             '[class*="article__content-body"],[data-component="article-body"]',
    'bloomberg.com':      '[class*="body-content"],[class*="article-body"]',
    default:              'article,.article-body,.paywall,[class*="article-body"],[role="article"],main,.story-body,.story-content'
  };

  // 2. Wait for target element to appear (polling)
  function waitForSelector(sel, timeout) {
    return new Promise(function(resolve) {
      const start = Date.now();
      (function check() {
        const el = document.querySelector(sel);
        if (el && el.innerText && el.innerText.trim().length > 50) return resolve(el);
  ...
  // 3. Try multiple selectors in priority order
  async function findBodyContainer() {
    const selList = bodySelectors[domain] ? bodySelectors[domain].split(',') : bodySelectors.default.split(',');
    for (const sel of selList) {
      const el = await waitForSelector(sel, 10000);
      if (el) return el;
    }
    return document.body || document.documentElement;
  }

  // 4. Unhide paywall elements
  ...
  // 5. Extract paragraphs (async-aware via synchronous fallback)
  var paras = [], seen = {};
  var containers = document.querySelectorAll(bodySelectors[domain] || bodySelectors.default);
  ...
  // 6. Fallback: scan all p tags if body container had too few
  if (paras.length < 3) {
  ...
  // 8. Section-organized article links
  var urlMap = {};
